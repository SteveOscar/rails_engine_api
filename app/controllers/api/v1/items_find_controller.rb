class Api::V1::ItemsFindController < Api::ApiController
  ...
  def index
    ...
    respond_with Item.where("lower(items.created_at) LIKE '#{params[:created_at].downcase}'") if params[:created_at]
    respond_with Item.where("lower(items.updated_at) LIKE '#{params[:updated_at].downcase}'") if params[:updated_at]
    respond_with Item.find(params[:id]) if params[:id]
  end
